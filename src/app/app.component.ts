import { Component, HostListener, ChangeDetectionStrategy, computed, signal, effect } from '@angular/core';
...
export class AppComponent {
  ...
  readonly currentPageTitle = signal('Resume Builder');
  readonly currentSection = signal('Talent Acquisition');
  ...
  readonly mainWrapperClasses = computed(() => ({
    'icon-only': !this.isMobile() && this.sidebarCollapsed(),
  ...
  private updateBreadcrumb() {
    ...
    if (url.includes('/resume')) {
      this.currentPageTitle.set('Resume Builder');
      this.currentSection.set('Talent Acquisition');
      return;
    }

    if (url.includes('/assessment')) {
      this.currentPageTitle.set('Candidate Assessment');
      this.currentSection.set('Talent Acquisition');
      return;
    }

    if (url.includes('/reports')) {
      this.currentPageTitle.set('Assessment Reports');
      this.currentSection.set('Talent Acquisition');
      return;
    }
  }
